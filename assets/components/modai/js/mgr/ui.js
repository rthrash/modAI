(() => {
    ...
    const createElement = (type, styleObj = null, textContent = '') => {
    ...
    const nlToBr = (content) => {
        return /<[^>]*>/g.test(content) ? content : content.replace(/\r\n|\n|\r/g, '<br>');
    }

    const createContentIframe = (providedContent, modal, customCSS = []) => {
    ...
    const createModal = (config) => {
        ...
        const buildModal = () => {
            ...
            // Create modal
            const chatModal = createElement('div', styles.chatModal);
        ...
        `;
            ...
            chatModal.abortController = null;
            chatModal.offsetX = null;
            chatModal.offsetY = null;
        ...
        const addAssistantMessage = (content, providedId = null) => {
            if (modal.chatMessages.style.display === 'none') {
                modal.chatMessages.style.display = 'block';
            }
            ...
            if (config.actions?.insert && typeof config.actions.insert === 'function') {
                ...
                insertBtn.addEventListener('click', () => config.actions.insert(history.getAssistantMessage(messageId), closeModal));
        ...
        const copyToClipboard = (messageId, copyBtn) => {
        ...
        const sendMessage = async (providedMessage, hidePrompt = false) => {
            ...
            try {
                const data = await modAI.executor.mgr.prompt.freeText(
                    {
        ...
        const history = modAI.chatHistory.init(
            config.key,
            (msg) => {
                if (msg.hidden) {
                    return;
            ...
            (msg) => {
                ...
                const iframe = msg.el.firstChild.firstChild;
